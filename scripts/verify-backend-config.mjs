...
for (const path of sourceFiles) {
  ...
  if (/service_role|sb_secret_/i.test(text)) {
    failures.push(`${file}: secret or service-role credential marker detected`)
  }
  if (/(sos_|oc_|luxe_|bp_|gt_)/.test(text)) {
    failures.push(`${file}: another app namespace was referenced inside THE LAW source`)
  }
}
...
for (const rpc of [
  ...
  if (!page.includes(rpc)) {
    failures.push(`src/app/page.tsx: required bounded RPC ${rpc} is missing`)
  }
}
if (/\.from\("rr_/.test(page)) {
  failures.push('src/app/page.tsx: direct rr_* table access detected; use bounded public RPCs')
}
...
console.log(`THE LAW backend isolation verified across ${sourceFiles.length} source files.`)
